...
export default function ChampionPool() {
  ...
  const [championName, setChampionName] =
    useState<{ titles: string[]; images: string; tags: string[] }[]>();
  ...
  function ChampionsImages() {
    ...
    return (
      <div
        className="grid h-64 grid-cols-6 overflow-y-auto
         p-6  text-xs "
      >
        {imagePaths.map((imagesUrl, index) => {
            ...
            <div className="flex flex-col items-center">
              {champion ? (
                <>
                  ...
                  <div className="">
                    {champion.names.replace(/([A-Z])/g, ' $1').trim()}
                  </div>
                </>
              ) : null}
            </div>
